...
type User = {
  ...
  storeName: string; storeImage: string;
};
...
type Client = {
  id: string; name: string; address: string; phone: string; email: string; image: string;
};

type SaleItem = { productId: string; name: string; qty: number; price: number; subtotal: number; };
type Sale = { id: string; date: string; clientId: string; clientName: string; items: SaleItem[]; total: number; };
...
export class HomePage {
  ...
  /* ====== Reportes (derivados) ====== */
  report = { totalSales: 0, totalRevenue: 0, totalItems: 0,
  ...
  /* ====== Auth ====== */
  switchAuth(mode: 'login'|'register') { this.authMode = mode; this.authError=''; }
  ...
  doLogin() {
    ...
    this.session = u; this.set(SESSION_KEY, u); this.authError = '';
  }

  doRegister() {
    const { username, password, storeName, storeImage } = this.registerForm;
    if (!username || !password || !storeName) { this.authError = 'Completa los campos obligatorios'; return; }
    const users = this.get<User[]>(USERS_KEY, []);
    if (users.some(u => u.username === username)) { this.authError = 'El nombre de usuario ya existe'; return; }
    ...
    this.authMode = 'login'; this.authError = '';
  }

  logout() {
    localStorage.removeItem(SESSION_KEY);
    this.session = null; this.section = 'dashboard';
  ...
  saveProduct() {
    ...
    this.products = list; this.set(PRODUCTS_KEY, list);
    this.resetProduct();
  }
  editProduct(p: Product) { this.productForm = { ...p }; }
  deleteProduct(id: string) {
    this.products = this.products.filter(p => p.id !== id);
    this.set(PRODUCTS_KEY, this.products);
    if (this.productForm.id === id) this.resetProduct();
  }
  resetProduct() { this.productForm = { stock: 0, costPrice: 0, salePrice: 0 }; }
  ...
  saveClient() {
    ...
    this.clients = list; this.set(CLIENTS_KEY, list);
    this.resetClient();
  }
  editClient(c: Client) { this.clientForm = { ...c }; }
  deleteClient(id: string) {
    this.clients = this.clients.filter(c => c.id !== id);
    this.set(CLIENTS_KEY, this.clients);
    if (this.clientForm.id === id) this.resetClient();
  }
  resetClient() { this.clientForm = {}; }
  ...
  addItemToSale() {
    this.saleError = '';
    const pid = this.saleAdd.productId; const qty = Number(this.saleAdd.qty || 0);
    if (!pid || qty <= 0) { this.saleError = 'Selecciona producto y cantidad válida'; return; }
    const p = this.products.find(x => x.id === pid);
    if (!p) { this.saleError = 'Producto no encontrado'; return; }
    if (qty > p.stock) { this.saleError = `Stock insuficiente. Disponible: ${p.stock}`; return; }

    // Si ya existe en la lista, acumula
    const existing = this.saleForm.items.find(i => i.productId === pid);
    if (existing) {
      if (existing.qty + qty > p.stock) { this.saleError = `Supera stock. Disponible: ${p.stock}`; return; }
      existing.qty += qty;
      existing.subtotal = existing.qty * existing.price;
    } else {
      this.saleForm.items.push({
        productId: p.id, name: p.name, qty, price: p.salePrice, subtotal: qty * p.salePrice
      });
    }
    this.saleAdd = { qty: 1 };
  }
  ...
  saveSale() {
    this.saleError = '';
    if (!this.saleForm.clientId) { this.saleError = 'Selecciona un cliente'; return; }
    if (!this.saleForm.items.length) { this.saleError = 'Agrega al menos un producto'; return; }

    // Validar stock final
    for (const it of this.saleForm.items) {
      const p = this.products.find(x => x.id === it.productId)!;
      if (it.qty > p.stock) { this.saleError = `Stock insuficiente para ${p.name}`; return; }
    }
    ...
    this.products = newProducts; this.set(PRODUCTS_KEY, newProducts);
    ...
    this.sales = list; this.set(SALES_KEY, list);
  ...
  private refreshReport() {
    ...
    const totalItems   = this.sales.reduce((a, s) => a + s.items.reduce((x, i) => x + i.qty, 0), 0);
    ...
    for (const [id, amount] of byClient.entries()) {
      if (!topClient || amount > topClient.amount) topClient = { name: clientNames.get(id) || id, amount };
    }
    ...
    for (const v of byProduct.values()) {
      if (!topProduct || v.qty > topProduct.qty) topProduct = { name: v.name, qty: v.qty };
    }
